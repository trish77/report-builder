$(function () {
  ...
  var assgTable = $('#assgTable').DataTable({
    ...
    processing: true,
    responsive: true,
    dom: '<"d-flex"i<"ms-auto"l>>rt<"d-flex"l<"ms-auto"p>>',
    language: {
      search: "_INPUT_",
...
// init assgCompleted chart block
 ...
 // completePercentWrapper.innerHTML(txtPercent);
 // console.log(completePercentWrapper);


  const assgNumPastDue = [ 35, 65 ];
  const assgNumPDPercent = assgNumPastDue[ 0 ] ;
  const dataPastDue = {
    labels: [ 'Past Due', 'Remaining' ],
    datasets: [ {
      data: assgNumPastDue,
      cutout: '80%',
      borderRadius: [ 10,  0 ],
...
// init assgCompleted chart block
  ...
  const assgOtherNum = [ 15, 10, 75 ];
  const assgOtherNumPercent = assgOtherNum[ 0 ] + assgOtherNum[ 1 ];
  const dataOther = {
    labels: [  'Exempt', 'Delinquent', 'Other' ],
    datasets: [ {
      data: assgOtherNum,
      cutout: '80%',
...
// init assgCompleted chart block
  ...
  $('#assgOtherWrapper .center-metric').append(assgNumPercent + '<sup><span style="font-size:30px">%</span></sup>');
  $('#assgPastWrapper .center-metric').append(assgNumPDPercent + '<sup><span style="font-size:30px">%</span></sup>');
  $('#assgCompletedWrapper .center-metric').append(assgNumPercent + '<sup><span style="font-size:30px">%</span></sup>');
